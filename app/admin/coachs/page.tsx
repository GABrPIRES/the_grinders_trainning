...
export default function AdminCoachListPage() {
  ...
  const [limit, setLimit] = useState(10); // Reativado
  const [page, setPage] = useState(1);     // Reativado
  const [total, setTotal] = useState(0);   // Reativado
  const [loading, setLoading] = useState(true);

  const fetchCoachs = async () => {
    setLoading(true);
    try {
      // 1. Enviamos os parâmetros de paginação e busca para a API
      ...
      // 2. A resposta da API agora é um objeto com 'users' e 'total'
  ...
  // 3. O useEffect agora depende de 'page' e 'limit' também
  useEffect(() => {
    fetchCoachs();
  }, [search, page, limit]);
