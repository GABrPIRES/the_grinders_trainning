...
export default function CreateWorkoutPage() {
  ...
  // NOVO ESTADO: Armazena os dados da semana
  ...
  const handleAddExercise = () => setExercises(prev => [...prev, { id: uuid(), name: "", sections: [{ id: uuid(), carga: undefined, series: undefined, reps: undefined, equip: "", rpe: undefined, pr: undefined, feito: false }] }]);
  const handleAddSection = (exerciseId: string) => setExercises(prev => prev.map(ex => ex.id === exerciseId ? { ...ex, sections: [...ex.sections, { id: uuid(), carga: undefined, series: undefined, reps: undefined, equip: "", rpe: undefined, pr: undefined, feito: false }] } : ex));
  ...
  const handleSectionChange = (exerciseIndex: number, sectionIndex: number, field: string, value: any) => {
    const updated = [...exercises]; const section = updated[exerciseIndex].sections[sectionIndex] as Section;
    if (field === 'feito') { section[field] = value; } else if (['carga', 'rpe'].includes(field)) { const parsed = parseFloat(value); section[field] = isNaN(parsed) ? undefined : parsed; } else if (['series', 'reps'].includes(field)) { const parsed = parseInt(value); section[field] = isNaN(parsed) ? undefined : parsed; } else { section[field] = value; }
    if (section.carga && section.reps && section.rpe) { const pr = calculatePR({ carga: section.carga!, reps: section.reps!, rpe: section.rpe! }); if (pr !== null) { section.pr = parseFloat(pr.toFixed(2)); } }
    ...
    try {
      const payload = {
        treino: {
          ...
          day: date,
          exercicios_attributes: exercises.map(ex => ({ name: ex.name, sections_attributes: ex.sections.map(sec => ({ carga: sec.carga, series: sec.series, reps: sec.reps, equip: sec.equip, rpe: sec.rpe, pr: sec.pr, feito: sec.feito })) })),
        },
      };
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <input type="number" placeholder="Duração (min)" className="text-neutral-600 border p-2 rounded w-full" value={duration} onChange={e => setDuration(e.target.value)} required />
          <input
            type="date"
            ...
            disabled={!weekData} // Desabilita o campo enquanto as datas não forem carregadas
            required
          />
        </div>

        {/* O resto do formulário (exercises.map) permanece idêntico */}
